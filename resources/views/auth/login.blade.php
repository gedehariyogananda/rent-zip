    ...
    <div class="hidden lg:flex lg:w-1/2 relative bg-brand-900 overflow-hidden flex-col justify-between p-12 text-white"
        ...
        <div class="relative z-10">
            <div class="flex items-center gap-3 mb-12">
                ...
                <span class="font-bold text-xl tracking-wide">{{ config('app.name', 'Rental Costum') }}</span>
            </div>
    ...
    <div class="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 sm:p-12 lg:p-24 bg-white">
        <div class="w-full max-w-md">
            <div class="mb-10">
                ...
                <p class="text-gray-500">Sign in to continue your cosplay journey.</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center">
                        <input type="checkbox" name="remember" class="rounded border-gray-300 text-brand-600 shadow-sm focus:border-brand-300 focus:ring focus:ring-brand-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm text-gray-600">Remember me</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a class="text-sm font-medium text-brand-600 hover:text-brand-500" href="{{ route('password.request') }}">
                ...
                <div class="text-center mt-6">
                    <p class="text-sm text-gray-600">
                        ...
                        <a href="{{ route('register') }}" class="font-bold text-gray-900 hover:text-brand-600 transition-colors">Register</a>
                    </p>
                </div>
            </form>
            ...
            <div class="mt-16 text-center">
                <p class="text-xs text-gray-400 uppercase tracking-wider">
                    &copy; {{ date('Y') }} {{ strtoupper(config('app.name', 'Rental Costum')) }}. ALL RIGHTS RESERVED.
                </p>
            </div>
        </div>
    </div>
