...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-3xl font-bold text-gray-800 mb-1">Edit Pengguna</h1>
        <p class="text-sm text-gray-500">Perbarui informasi data pengguna di sistem.</p>
    </div>
</div>
...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="p-8">
        ...
        <div class="space-y-6">
            ...
            {{-- Password --}}
            <div>
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-2">Password <span class="text-gray-400 font-normal">(Opsional)</span></label>
                <div class="relative">
                    <input type="password"
                           id="password"
                           name="password"
                ...
                <p class="mt-2 text-xs text-gray-500">Isi hanya jika Anda ingin mengubah password pengguna ini (min. 8 karakter).</p>
                @error('password')
                    <p class="mt-2 text-sm text-red-500 flex items-center gap-1">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
