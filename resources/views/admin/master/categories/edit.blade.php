...
<div class="mb-8">
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-3">
        <a href="{{ route('admin.master.categories.index', ['type' => $category->type]) }}" class="hover:text-brand-600 transition-colors">
            {{ $category->type === 'costum' ? 'Kategori Kostum' : ($category->type === 'pengeluaran' ? 'Kategori Pengeluaran' : 'Kategori Pengurangan Stok') }}
        </a>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('admin.master.categories.update', $category->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <label class="flex items-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-colors
                                  {{ old('type', $category->type) === 'costum' ? 'border-brand-600 bg-brand-50' : 'border-gray-200 hover:border-gray-300' }}">
                        <input type="radio" name="type" value="costum"
                               {{ old('type', $category->type) === 'costum' ? 'checked' : '' }}
                               class="text-brand-600 focus:ring-brand-500">
                        <span class="text-sm font-semibold text-gray-700">Kostum</span>
                    </label>
                    <label class="flex items-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-colors
                                  {{ old('type', $category->type) === 'pengeluaran' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300' }}">
                    ...
                    <label class="flex items-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-colors
                                  {{ old('type', $category->type) === 'maintenance' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300' }}">
                        <input type="radio" name="type" value="maintenance"
                               {{ old('type', $category->type) === 'maintenance' ? 'checked' : '' }}
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.master.categories.index', ['type' => $category->type]) }}"
