...
<div class="mb-8 flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold text-brand-800 mb-2">Tambah Transaksi Sewa</h1>
        <p class="text-gray-500">Buat pesanan baru untuk pelanggan yang datang langsung.</p>
    </div>
    <a href="{{ route('admin.orders.index') }}" class="flex items-center px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors shadow-sm">
...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden" x-data="orderForm()">
    ...
    <form action="{{ route('admin.orders.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
        @csrf
        ...
        <div class="mb-10">
            ...
            <div class="flex gap-4 mb-6">
                <label class="flex items-center gap-2 px-5 py-3 rounded-xl border-2 cursor-pointer transition-colors"
                       :class="userType === 'existing' ? 'border-brand-600 bg-brand-50' : 'border-gray-200 hover:border-gray-300'">
                    <input type="radio" name="user_type" value="existing" x-model="userType" class="text-brand-600 focus:ring-brand-500">
                    <span class="text-sm font-semibold text-gray-700">Pilih dari Member</span>
                </label>
                <label class="flex items-center gap-2 px-5 py-3 rounded-xl border-2 cursor-pointer transition-colors"
                       :class="userType === 'new' ? 'border-brand-600 bg-brand-50' : 'border-gray-200 hover:border-gray-300'">
                    <input type="radio" name="user_type" value="new" x-model="userType" class="text-brand-600 focus:ring-brand-500">
                    <span class="text-sm font-semibold text-gray-700">Daftarkan Member Baru</span>
                </label>
            </div>

            {{-- Existing User Dropdown --}}
            <div x-show="userType === 'existing'" class="max-w-xl">
                <label class="block text-sm font-bold text-gray-700 mb-2">Cari Member <span class="text-red-500">*</span></label>
                <select name="user_id" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500" :required="userType === 'existing'">
                    <option value="">-- Pilih Member --</option>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}">{{ $user->username }} ({{ $user->phone ?? 'No Phone' }})</option>
                    @endforeach
                </select>
            ...
            {{-- New User Form --}}
            <div x-show="userType === 'new'" class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Username <span class="text-red-500">*</span></label>
                    <input type="text" name="username" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" :required="userType === 'new'">
                ...
                <div>
                    ...
                    <input type="email" name="email" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" :required="userType === 'new'">
                    @error('email') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Password <span class="text-red-500">*</span></label>
                    <input type="password" name="password" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" :required="userType === 'new'">
                    @error('password') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nomor WhatsApp <span class="text-red-500">*</span></label>
                    <input type="text" name="phone" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" :required="userType === 'new'">
                    @error('phone') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Alamat Lengkap <span class="text-red-500">*</span></label>
                    <textarea name="address" rows="2" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 resize-none" :required="userType === 'new'"></textarea>
                    @error('address') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nomor Darurat (Keluarga) <span class="text-red-500">*</span></label>
                    <input type="text" name="no_darurat" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" :required="userType === 'new'">
                    @error('no_darurat') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Upload KTP <span class="text-red-500">*</span></label>
                    <input type="file" name="ktp_url" accept="image/*" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" :required="userType === 'new'">
                ...
                <template x-for="(item, index) in items" :key="index">
                    <div class="flex items-end gap-4 p-4 border border-gray-200 rounded-2xl bg-white">
                        <div class="flex-grow">
                            ...
                            <select x-model="item.id" @change="updatePrice(index)" :name="`costums[${index}][id]`" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500" required>
        ...
        <div class="pt-6 border-t border-gray-100 flex items-center justify-end gap-4">
            <a href="{{ route('admin.orders.index') }}" class="px-6 py-3 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors">
                Batal
            </a>
